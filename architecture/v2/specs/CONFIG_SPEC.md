...
## 1. ブランチ管理機能 (Branch Management)

リポジトリのブランチ管理機能を提供する画面です。
...
- **Target Branch** (Required):
  ...
  - 説明: このブランチ設定は、配下の全てのコンテンツ設定に適用されます。

#### ブランチ管理機能 (Branch Management)

ターゲットブランチ (Target Branch)
を変更・保存すると、デフォルトブランチとの差分を確認・操作できます。

- **Unmerged Commits**:
  - ターゲットブランチに含まれるが、デフォルトブランチにはマージされていないコミットの一覧を表示します。
- **Create Merge Pull Request**:
  - Unmerged Commits
    が存在する場合、デフォルトブランチへのマージ用プルリクエストを作成する UI
    を提供します。
  - **Title**: PRのタイトルを入力（必須）。
  - **Create PR**: GitHub API を使用して PR を作成し、作成された PR
    へのリンクを表示します。
