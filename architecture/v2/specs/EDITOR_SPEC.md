...
- **Right**: ステータス表示エリア
  ...
  - **Draft Label**:
    ローカルドラフト（未保存変更）が存在する場合に表示（オレンジ色）。
...
- コンテンツ設定 (Config)
  ...
  - **Design**:
    シンプルなキー・バリュー形式のテキスト入力フィールドとして表示します。
  - **Widgets**:
    - 現在のバージョンでは、データ型に関わらず一律で **テキスト入力 (`string`)**
      として扱います。
    - 将来的にリッチなウィジェット（Datepicker, Select等）の再導入を検討します。
...
- **Drag & Drop**:
  - エディタ領域への画像ファイルのドロップを受け付け、自動的にアップロード待機リストに追加し、Markdown
    にリンクを挿入します。
  - **Images Nearby**
    リストからの画像アイテムのドラッグ＆ドロップを受け付け、ドロップ位置に
    Markdown リンク (`![name](name)`) を挿入します。
- **Toolbar**: 太字、斜体、リンク、画像挿入などの標準ツールバー。

### 3. サイドバー (Sidebar Column)

> **Note**: YAML 編集モード (`.yml`/`.yaml`)
> の場合、サイドバー内のコンテンツ（画像管理など）は非表示となりますが、レイアウトの一貫性を保つためにカラム領域（4
> wide）自体は確保（空白表示）されます。
...
1. **Nearby (同階層)**:
   ...
   - リポジトリのターゲットブランチを参照します。ディレクトリが存在しない（404）場合は、エラーとせず空リストとして扱います。
   - **Click Action**:
     サムネイルまたはファイル名をクリックすると、**プレビューモーダル**を表示します。
     - モーダルは画面中央に表示され、拡大画像を確認できます。
   - **Drag & Drop**:
     リスト項目をエディタへドラッグ＆ドロップすることで、Markdownリンクを挿入できます。
2. **Pending (アップロード待ち)**:
   - 今回の編集セッションで追加されたが、まだ GitHub
     にコミットされていない画像。
   - サムネイル表示。
   - Nearby
     と同様に、クリックでプレビュー（Base64表示）、ドラッグ＆ドロップでリンク挿入が可能。
   - 削除ボタン（アップロード取り消し）。
3. **Upload Area**:
...
1. **State Loading**: `localStorage`
   ...
   - **Draft Key Strategy**: 新規作成時 (`mode="new"`)
     であっても、入力されたスラッグ名（articleName）を使用してキーを生成し、永続化します（`__new__`
     キーは使用しません）。これにより、新規作成直後も一意に特定・復元可能です。
2. **Remote Fetching**: GitHub API から最新のコンテンツを取得。
   - **404 Handling**:
     コンテンツがリモートに存在しない（404）場合でも、ローカルドラフトが存在すれば正常として扱い、ドラフト内容を表示します（新規ドラフトの継続編集シナリオ）。ローカルドラフトもなくリモートも404の場合はエラー「Content
     not found」とします。
3. **Conflict Resolution**:
   - ドラフトが存在する場合: ドラフトを優先表示し、UI に「Draft」と表示。
...
- **Left Actions**:
  ...
  - **Commit Message Input**:
    - 保存時にコミットメッセージの入力が必須となります。
    - 入力がない場合、保存ボタンは非活性化されます。
...
1. **Validation**:
   - **必須項目チェック**: `config.fields` で `required: true`
     とされたフィールドが空でないか確認します。
   - **コミットメッセージ**: 入力されているか確認します。
2. **Image Processing**:
...
- **適用条件**:
  - ファイル拡張子が `.yml` または `.yaml` である（※JSONは現在未サポート）。
  - `path` で個別に指定された Singleton コンテンツ等で利用されます。
...
2. **List Items Area (Sortable List)**:
   - 配列の各要素を行単位でレンダリングします。
   - **Right Controls**:
     - 各行の右側にコントロール用のカラムを配置します。
     - **Index Label**: アイテム番号（例: `#1`）を表示。
     - **Remove Button**: 削除ボタン。
   - **Sortable**:
     - (※
       現在のバージョンではドラッグハンドルは削除され、シンプルなリスト表示となっていますが、将来的にDnD復活の可能性があります)
   - **Item Editor**:
     - `config.fields`
       で定義されたスキーマに基づき、要素内のフィールドをテキスト入力として編集します。
