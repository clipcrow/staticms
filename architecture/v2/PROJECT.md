...
- **自動保存の実施:**
  編集中の状態（コンテンツと新規画像ファイル）および**関連するPR情報**は、リポジトリへの反映前に**ブラウザのローカルストレージ**に**単一の状態オブジェクト**として**常に自動で保存**されます。
  キー接頭辞: `staticms_draft_`
- **セッション持続:**
  次回同じコンテンツを開いた際に、この**状態（ドラフトおよびPR情報）**が復元されます。
...
- **ロック条件:**
  ...
  ただし、**ローカルに未保存の変更（ドラフト）**がある場合は、編集作業の継続を優先するためロックされません。
- **UIの挙動（ロック中）:**
  1. コンテンツ編集画面には、**常にプルリクエストが存在することを示すメッセージ**が表示され、コンテンツ全体が**読み取り専用**（編集ロック状態）となります。
  2. メッセージには、オープン中のPRのリンクが表示されます。
- **ロックの解除とリセット:**
  PRの「マージ」または「クローズ」イベントをWebHookで受信した場合、コンテンツの**編集ロックを解除**します。
  さらに、**ローカルの状態オブジェクトからPR情報を削除**し、コンテンツ自体が最新のリモート内容と一致するか（Cleanか）を確認します。
  - **Clean**: オブジェクト全体を削除（完全なリセット）。
  - **Dirty**: 未保存の変更としてドラフトを保持。
